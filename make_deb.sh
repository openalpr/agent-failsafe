...
rm -Rf debian/usr/share/openalpr-agent-failsafe

mkdir -p debian/usr/share/openalpr-agent-failsafe/
mkdir -p debian/etc/init.d/



cp $CUR_DIR/openalpr_agent_failsafe.py debian/usr/share/openalpr-agent-failsafe/
cp $CUR_DIR/requirements.txt debian/usr/share/openalpr-agent-failsafe/
cp $CUR_DIR/deploy/openalpr-agent-failsafe debian/etc/init.d/

# Insert the git hash into settings.py
echo "$LAST_COMMIT_HASH" > debian/usr/share/openalpr-agent-failsafe/githash
...
DEB_FILE=openalpr-agent-failsafe_${VER_WITH_BUILD}.deb
